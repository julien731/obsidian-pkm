---
type: resource
created: 2026-03-10
summary: "360 feedback review for Heri from the 2025 review cycle"
tags:
  - performance-review
  - nimble
related:
  - "[[people/Heri]]"
---

# 360 Review: Heri (2025)

## Summary

**Strengths:**
- Responsive and accessible (4 reviewers) — quick to answer questions and unblock the team
- Proactive product improvement (2) — actively suggests UX enhancements and new tools like Microsoft Clarity
- Open to team input (2) — receptive to developer suggestions and feedback
- Clear requirements and realistic expectations (1) — structured stories with minimal ambiguity, no unnecessary pressure

**Areas for improvement:**
- AI-generated ticket quality (3) — tickets created with AI lack verification, can be unclear, overly lengthy, or confusing for developers
- Acknowledgment of comments (1) — team unsure if their questions on stories have been seen; even a quick emoji reaction would help

## What Went Well

### From S M Abu Taher

- Responds quickly to queries.
- Communicates effectively about project requirements.

### From Dang Phuong Nam

- I really like how proactive and helpful Heri is. Whenever I have question regarding a user story, he will be very patient and help me clear up any issue!
- Heri is very responsive to feedback from developers whether it's about sprint scope or suggestions for features/bugs, which helps create a better and more proficient team workflow.

### From Khanh

- He’s very positive and energetic
- Requirements are generally clear and structured, so we’re rarely starting with major ambiguity. He keeps expectations realistic without putting unnecessary pressure on the team.
- He consistently looks for ways to improve the app. He also actively reviews flows, spots errors, and suggests practical UX/UI improvements like settings for guests, crop center in HDP images,...
- He’s also open to new ideas and actively looks for ways to improve the product. He doesn’t just execute — he suggests new features and enhancements that help move the project forward like Microsoft Clarity, consent modal, carousel for home page...

### From Suphanat Jarukulgowit

Heri has been managing the team very effectively. He has a strong understanding of how the application works, including important flows such as the payment workflow, which is critical to the business.

He also leverages AI tools to support his workflow, such as generating tickets, which improves the team’s productivity. Additionally, he consistently communicates the direction of each sprint and shares client feedback, helping the team stay aligned with goals and expectations.

### From Khang Bui

Heri is very supportive with clarifying concerns and uncertainty. He is also open minded to adopt suggestions made by the development team

### From Julien (me)

- Great job leading easyHotel. The project is moving forward smoothly.
- You're doing well with pushing the right plans and initiatives with the client.
- You're doing well with navigating the organization and the rapidly changing landscape.
- Nice job with upselling additional resources.
- Question: what are your objectives? Which direction would you like to take? IC/Principal?

## What Could Be Better

### From S M Abu Taher

- Shortcut story descriptions could be more detailed, especially for new joiners on the team who need more context.

### From Dang Phuong Nam

- It would be really helpful if you could acknowledge when you've seen our comments or questions on user stories, even with a quick emoji reaction or a short reply. Sometimes it's unclear if a comment has been seen, which can lead to us following up on Slack just to confirm you're aware of it.

### From Khanh

At the moment, I don’t see any major gaps that need improvement. He’s performing strongly in both execution and collaboration

### From Suphanat Jarukulgowit

While using AI to generate tickets is very helpful, the context in some tickets can sometimes be unclear, overly lengthy, or not direct enough.

To improve this, it would be beneficial to ensure that tickets are more concise and structured — for example, clearly stating the objective, scope, and expected outcome at the beginning.

### From Khang Bui

Some of the tickets created seems to be done via AI and is a bit lack of verifying thus sometimes it could cause confusion when a developer reads and works on it. He could spend some more time to make sure the ticket is clearer

### From Julien (me)

I would love to see you more involved with the Product team in general.

You have many good ideas, you tinker with lots of stuff, but you rarely share. Your contributions would be highly valuable.

## Phase 2: AI Synthesis

### Patterns Across Reviewers

**Strong consensus (4+ reviewers):** Heri is highly responsive and accessible. Taher, Nam, Khanh, Khang, and Suphanat all highlight his availability to clarify questions and unblock developers. This is his clearest, most consistently recognized strength.

**Moderate consensus (2-3 reviewers):** Proactive product thinking — Khanh and Suphanat both note he doesn't just execute but actively suggests improvements (Microsoft Clarity, UX enhancements, spotting errors in flows). Openness to developer input — Nam and Khang both call out that he listens to and adopts team suggestions.

**AI-generated ticket quality (3 reviewers):** Taher, Suphanat, and Khang independently flag that tickets created via AI are unclear, too long, or insufficiently verified. This is the strongest improvement signal — it's specific, repeated, and actionable.

### Blind Spots

- **Heri likely underestimates the impact of AI-generated tickets.** Three of five developer reviewers flagged it, suggesting it's a recurring friction point, not an isolated complaint. He may see AI ticket creation as a productivity gain without realizing it shifts cognitive load onto developers who have to decode unclear requirements.
- **Acknowledgment gap is small but real.** Nam's comment about not knowing if comments are seen points to a low-effort fix (emoji reactions) that would meaningfully reduce follow-up Slack messages. Heri may not realize this creates friction because the delay is invisible to him.
- **Conspicuously absent: no one mentions mentoring, delegation, or developing others.** For someone managing a team effectively (Suphanat's words), there's no feedback about growing team members' skills or ownership. This could be a blind spot or simply not yet expected at his level — worth exploring.

### Strength-to-Growth-Area Connections

- **Proactive product thinking ↔ Not sharing broadly.** Julien's observation that Heri tinkers and has good ideas but rarely shares connects directly to the strength Khanh and Suphanat praise. The strength exists but is contained within easyHotel — expanding it to the broader Product team would multiply its value.
- **AI tool adoption ↔ Ticket quality.** Suphanat praises Heri's use of AI tools as a strength. Three reviewers flag AI-generated tickets as a problem. The same behavior is both a strength (willingness to adopt new tools) and a weakness (insufficient quality control on the output). The fix isn't to stop using AI — it's to add a verification step.

### Development Recommendations

1. **Establish a personal review step for AI-generated tickets.** Before publishing any AI-created ticket, re-read it as if you're a developer seeing it for the first time. Check: Is the objective clear in the first sentence? Is the scope defined? Can you remove 50% of the text without losing meaning? Target: no developer should need to ask a clarifying question on a well-written ticket.

2. **Adopt a "seen it" habit for story comments.** Add an emoji reaction or short reply within 4 hours of any comment on a story you own. This costs seconds but eliminates the uncertainty that drives follow-up Slack messages.

3. **Share one product insight per sprint with the broader Product team.** Pick one thing you discovered, tested, or learned (e.g., a Clarity finding, a UX pattern, a client interaction insight) and post it in the Product team channel. This directly addresses Julien's feedback about contributing more broadly and leverages the proactive thinking your team already values.

4. **Start documenting decisions, not just tickets.** Given Suphanat's note about Heri's strong understanding of critical flows like payments — this knowledge should be externalized. When making a product decision on a key flow, write a brief "why we chose X" note. This builds team resilience and positions Heri as a knowledge leader.