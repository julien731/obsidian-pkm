---
type: resource
created: 2026-03-10
summary: "360 feedback review for Andrew from the 2025 review cycle"
tags:
  - performance-review
  - nimble
related:
  - "[[people/Andrew]]"
---

# 360 Review: Andrew (2025)

> **TL;DR Executive Summary**
>
> **Recommendation:** Conditional increase — publish designer leveling framework by end of Q2
>
> **Key Strengths:** Proactive AI and tool adoption champion (Claude Code, Figma MCP); approachable and supportive leadership creating psychological safety; consistently upholds high design standards; strong knowledge sharing and initiative.
>
> **Development Areas:** Career path and leveling clarity missing for design team (3 reviewers); team growth not visibly accelerating — needs more structured mentoring; strategic visibility in product discussions could be stronger.
>
> **Rationale:** Excellent on craft and team support, but core Lead responsibility (career framework) remains unfulfilled. Condition addresses the blocker before compensation adjustment.

## Summary

**Strengths:**
- Proactive AI and tool adoption (3 reviewers) — drives AI experimentation, new workflows (Claude Code, Figma MCP), and encourages the team to adopt them
- Approachable and supportive leadership (4) — creates a transparent, safe environment for peer reviews, feedback, and raising concerns
- High design standards (3) — consistently upholds quality through structured reviews, clean UI, and strong UX focus
- Knowledge sharing and initiative (3) — proactively explores and shares new tools, best practices, and design thinking approaches
- Clear task assignment and direction (2) — provides detailed context, expectations, and reasoning behind work

**Areas for improvement:**
- Career path and leveling clarity (2) — no structured framework defining expectations, skills, and promotion criteria per designer level
- Team growth acceleration (1) — designers are good but their growth isn't visibly accelerating; more structured mentoring needed
- Strategic visibility (1) — could engage more in product strategy discussions and communicate high-level design vision
- Hiring alignment (1) — portfolio evaluation criteria are subjective; a scoring card or calibration sessions would improve sourcing efficiency

## What Went Well

### From Kade

Andrew is highly approachable and collaborative. I feel comfortable openly discussing hiring strategy, interview feedback, portfolio evaluations, and even other matters with him. He is also responsive and willing to clarify expectations when asked.

### From Prajna Vira

When the team needed strong design standards and operational clarity, Andrew consistently stepped up to uphold high standards across craft, systems thinking, and team performance. He actively drove design quality through structured reviews, strengthened the UX strategy by advocating for problems clarity, and maintained operational excellence in execution. He played a key leadership role by advocating for and showcasing the use of AI to elevate designers productivity, safeguarding performance standards across the team, and building a transparent feedback culture that helped identify improvement areas early. The team maintained a high design bar, improved clarity in feedback loops, and operated with stronger consistency and accountability as a result.

### From Got

- He consistently produces clean, polished UI that looks professional.
- He focuses on UX and how people will actually use the product, not just visuals.
- He proactively suggests ideas and improvements instead of only executing what requests.

### From taus daengkul

The Big picture Feedback - Andrew always gave me the feedback that can expand the scope more than i expected sometime it helps me have a new perspective for my solution

Always comes with new knowledge - The world is spining so fast and Andrew always suggest me to touch the new workflow like claude-code, Figma MCP, etc. in the other hands He make me still believe in the designer value that can combine to the new AI workflow.

### From Mai

- Very supportive. Listen to the problem or my stupid complaint and try to support and give very useful advice, or assist in solving the issue.
- Patient and good listener. Try to understand what I try to explain.
- Passionate about improving the team members' skills.
- Try to find a way to work to support the team.

### From Thien

One thing I really appreciate is how clear and structured he is when assigning tasks. He consistently provides detailed direction, sets clear expectations, and explains the context behind the work. That clarity makes it much easier for me to prioritize, execute confidently, and avoid unnecessary back-and-forth. It also helps reduce ambiguity and speeds up delivery.

I also highly value how he supports and represents the design team when working with clients. In the JB Loyalty Project, the client frequently requested design changes based on their internal decisions while overlooking our team’s recommendations. He stepped in and reinforced with clients that our role as UX/UI advisors is not just to execute, but to surface risks, inconsistencies, and best-practice considerations. He encouraged creating space for healthy discussion before final decisions were locked, ensuring that UX concerns and alternatives could be properly presented. That intervention not only protected the quality of the work but also reinforced the credibility of the design team. It created psychological safety for us to raise concerns and focus on delivering thoughtful solutions rather than simply reacting to changes.

In addition, he proactively explores new tools, ideas, and best practices, and takes the initiative to share them with the team. Whether it’s workflow improvements or design thinking approaches, his knowledge-sharing helps us continuously improve how we work. It shows strong leadership and commitment to raising the overall standard of the team.

### From Laura

- Proactively encourages AI experimentation and framework optimization to increase team speed and output quality.

- Improved cross-project alignment by successfully adding much-needed structure to the Okya management of DPRs.

- Cultivates an open environment for peer reviews, ensuring team members have the support and "extra eyes" needed to solve complex challenges.

- Maintains a transparent culture where designers feel safe sharing concerns and addressing project roadblocks early.

### From Julien (me)

- Good job pushing the team to learn and get involved with AI.

## What Could Be Better

### From Kade

Design hiring is very subjective, and at times there is a misalignment between what I anticipate he would like in a portfolio and his actual preferences. This makes it more challenging to pre-filter portfolios effectively and can slow down the funnel. A structured calibration session or clearer portfolio scoring card could significantly improve alignment. Also, I believe there is an opportunity for deeper alignment around the current designer talent market. Having more structured discussions on market availability, salary benchmarks, and typical portfolio standards would help us calibrate expectations and make sourcing more efficient.

### From Prajna Vira

Andrew has been an empathetic and strong role model for designers. Moving forward, it would be even more impactful if Andrew could demonstrate greater visibility and actively engage in product strategy discussions, helping shape and communicate the high-level design vision.

### From Got

As a lead, I expect more visible improvement from the rest of the team. Our designers are good, but their growth isn’t accelerating.

He should spend more time mentoring, giving structured feedback, and helping bridge the gap between his skill level and the other designers.

### From taus daengkul

We can try to expand the connection with the other designer community that will help us increase the knowledge and can learn from each other.

### From Thien

One opportunity I see for further strengthening the team is around career path clarity and leveling structure for designers. Currently, the expectations between different UX/UI levels are not very transparent, and there aren’t clearly defined objectives or competencies that designers can actively work toward in order to grow or be promoted.

Introducing a more structured leveling framework — outlining skills, ownership expectations, impact scope, and promotion criteria for each level — could provide clearer direction for individual growth. It would also help align performance expectations, support more objective evaluations, and increase motivation across the team.

Given his leadership and understanding of the team’s strengths, I believe he is well positioned to define and formalize this structure in a way that supports both individual development and long-term team scalability.

### From Laura

While Andrew is great at project-level support, we could benefit from more structured, long-term career development planning.

### From Julien (me)

- You need to be more critical and keep the bar higher when hiring.
- You need to start thinking about the design work more holistically in the overall product building process.
- You need to become way more strict with the usage of branding assets at every level, including sales. E.g., we have many duplicated presentations leading to discrepancies. This is more of a new directive than a failure to be fair.

## Phase 2: AI Synthesis

### Patterns Across Reviewers

**Strong consensus (4+ reviewers):** Andrew creates a psychologically safe, approachable environment. Prajna Vira, Mai, Thien, Laura, and Kade all describe variations of feeling supported, heard, and safe raising concerns. This is his most consistently cited strength.

**Strong consensus (3+ reviewers):** AI and tool adoption leadership. Taus, Laura, and Julien all highlight his drive to push the team toward new workflows (Claude Code, Figma MCP, AI experimentation). He doesn't just adopt tools himself — he actively pushes the team to use them.

**Strong consensus (3+ reviewers):** High design standards and quality focus. Prajna Vira, Got, and Thien cite structured reviews, clean UI, strong UX thinking, and defending design quality with clients.

**Convergent concern (3 reviewers):** Career development and leveling structure is missing. Thien raises it most explicitly (no leveling framework, no promotion criteria), Laura echoes it (no long-term career development planning), and Got frames it as designers not growing fast enough. These three are describing the same gap from different angles.

### Blind Spots

**Andrew likely underestimates how much his team needs structured growth paths.** He invests in tools, standards, and day-to-day support — all visible. But career trajectory clarity is invisible until someone leaves or stagnates. Three reviewers flagging this independently suggests the gap is felt more broadly than he realizes.

**His supportiveness may be masking insufficient challenge.** Got's comment is pointed: "Our designers are good, but their growth isn't accelerating." Mai and Thien praise patience and listening. There may be a pattern where Andrew is so approachable and supportive that he under-pushes people. Being a good listener is not the same as being a demanding mentor.

**He may not see the gap between his own strategic capability and how he's perceived at that level.** Prajna Vira suggests he should engage more in product strategy discussions and communicate high-level design vision. Julien's feedback about thinking more holistically in the product building process points the same direction. Andrew operates well at the craft and team level but hasn't established himself as a strategic voice.

### Strengths Connected to Growth Areas

- **High approachability → under-challenging the team.** The safe environment is real and valuable. But the same instinct that makes Andrew patient and supportive may make him avoid the uncomfortable conversations that accelerate growth. Got's feedback ("their growth isn't accelerating") is the cost side of this strength.

- **Strong craft standards → insufficient strategic visibility.** Andrew upholds quality at the execution level — defending UX with clients (Thien's JB Loyalty example), maintaining clean UI, running structured reviews. But this focus on craft-level excellence may be absorbing energy that could go toward shaping product direction at a higher level. Prajna Vira and Julien both want him operating more strategically.

- **Tool and AI adoption leadership → potential gap in fundamentals.** Andrew is the team's champion for new tools and workflows, which is genuinely valuable. But new tools don't solve the structural gaps (career paths, leveling, mentoring rigor). There's a risk of investing energy in shiny workflow improvements while the foundational people infrastructure remains undefined.

### Development Recommendations

1. **Build and publish a designer leveling framework by end of Q2.** Define 3-4 levels with explicit expectations for craft skill, ownership scope, strategic thinking, and communication. Share it with the team and use it in 1:1s. This directly addresses the concern from Thien, Laura, and Got, and gives Andrew a tool for the harder growth conversations.

2. **Shift 1:1s from support mode to challenge mode.** For each direct report, identify one specific skill gap and create a concrete development goal with a deadline. Track it monthly. "Support" should mean helping people grow faster, not just making them comfortable. Got's feedback is the metric: are designers measurably better than they were 6 months ago?

3. **Take an active seat in product strategy for one project this quarter.** Not just defending design decisions after they're made (which he does well), but shaping the product direction upstream — participating in roadmap discussions, presenting design-led product hypotheses. Start with one project to build the muscle. Prajna Vira and Julien are both asking for this.

4. **Establish branding governance.** Audit existing sales and marketing materials for design inconsistencies. Create a checklist or approval gate for external-facing materials. This is a new directive from Julien, not a past failure — but it's a concrete opportunity to demonstrate strategic design leadership beyond the product team.

5. **Align with Kade on hiring evaluation criteria.** Create a portfolio scoring rubric with 4-5 explicit criteria and weights. Run a calibration session using 3 past portfolios to test alignment. This is a quick win that removes a specific friction point Kade raised.